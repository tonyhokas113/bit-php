...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if (isset($_GET['logout'])) {
        ...
        setMessage('Ate');
        header('Location: http://127.0.0.1/bit-php/13/login.php');
        die;
    }
    //1. Rodyti forma
} else {
    ...
    foreach ($users as $user) {
        if ($user['name'] == $_POST['name']) {
            if ($user['pass'] == md5($_POST['pass'])) {
                ...
                header('Location: http://127.0.0.1/bit-php/13/member.php');
                die;
            }
        }
    }
...
<body>
    ...
    <h1>Login</h1>
    <form action="http://127.0.0.1/bit-php/13/login.php" method="post">
